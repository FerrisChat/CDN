...
pub async fn upload(ContentLengthLimit(mut multipart): ContentLengthLimit<Multipart, { MAX_CONTENT_LENGTH }>) -> Result<Json<UploadResponse>, CdnError> {
    if let Ok(Some(mut field)) = multipart.next_field().await {
        ...
        let file_hash = String::from(Hash::hash(&buffer[..]));

        let ext = multipart.filename.unwrap_or_else(|| ErrorJson::new_400("No file name provided".to_string()).into()).split('.').last().unwrap_or_else(|| ErrorJson::new_400("No file extension found".to_string()).into());
        ...
        encoder.write_all(&buffer).await.map_err(|e| ErrorJson::new_500(format!("Something went wrong during compression: {:?}", e), true, None).into());
        encoder.shutdown.await.map_err(|e| ErrorJson::new_500(format!("Something went wrong during compression: {:?}", e), true, None).into());

        fs::write(path, &compressed[..]).await.map_err(|e| ErrorJson::new_500(format!("Something went wrong while saving file: {:?}", e), true, None).into());
